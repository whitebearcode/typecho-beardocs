...
margin-left:20px;
                    ...
                    '<script>_wpUtilSettings={ajax:{url: "'. Common::url('/bdoptions/ajax', Helper::options()->index).'"}};window.ajaxurl=_wpUtilSettings.ajax.url;</script>';
        ...
        public static function get_enqueue_script($old = null)
        {
            ...
            if ($loads_statics) { // if is plugin
                ...
                $script = '<script src="' . self::include_plugin_url('assets/js/lodash.min.js') . '?v=1"></script>' .
                    ...
                    '<script src="' . self::include_plugin_url('assets/js/backend.js') . '?v=1"></script>' .
...
//            self::include_plugin_file('views/welcome.php');
            ...
            $form_action = Helper::security()->getIndex('/bdoptions/save?plugin=' . $pluginName);
            ...
            if (class_exists('CSF_Options') && !empty(self::$args['admin_options'])) {
                foreach (self::$args['admin_options'] as $key => $value) {
                    ...
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        update_bd_key_params($key, $op_obj);
...
//            do_action('csf_loaded');
        ...
        {
            $val = new \Typecho\Widget\Helper\Form\Element\Hidden('bdOptionsHide',
                NULL,
                "bdOptionsHide",
                'bd选项框架隐藏val',
                '');
...
//      $directory_uri  = set_url_scheme( $directory_uri, $protocol_uri );
        ...
        {
            // include router
            self::include_plugin_file('classes/bdRouter.class.php');
